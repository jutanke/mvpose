...
class GraphPartitioning:
    ...
    def __init__(self, points3d, limbs3d, params, debug=False):
        """
        ...
        max_radius = params.gp_max_radius
        min_nbr_joints = params.min_nbr_joints
        min_joint_distance = params.min_joint_distance
        ...
        # ===========================================
        for jid1 in range(n_joints - 1):
            for jid2 in range(jid1+1, n_joints):
                assert jid1 != jid2
                ABdistance = gm.calculate_distance_all4all(
                    points3d[jid1], points3d[jid2],
                    max_distance=min_joint_distance,
                    min_distance=0,
                    AB_are_the_same=False)
                As = ABdistance[:, 0].astype('int32')
                Bs = ABdistance[:, 1].astype('int32')
